...
- `docker-compose.yaml` starts Postgres + pgAdmin, Elasticsearch + Kibana, and Grafana.
  ...
  docker compose up -d
  ```
  Postgres listens on `localhost:5434`; pgAdmin on `http://localhost:8080` (admin/admin). Elasticsearch on `http://localhost:9200`, Kibana at `http://localhost:5601`, Grafana at `http://localhost:3000` (admin/admin).
- uv is used to manage Python packages and environments (`pyproject.toml`/`uv.lock` tracked in-repo). Install via `pip install uv` and initialize via `uv init`. 
